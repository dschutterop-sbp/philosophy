...
$("#prepare").onclick = () => withBusy($("#prepare"), "Preparing…", async () => { try { resetDraftView(); $("#status").textContent = "Preparing…"; const input = context(); trace("Context builder", mode === "live" ? "Calling weather, calendar, operations, and publishing-history sources." : "Recording supplied scenario provenance.", "api", { contextBuilder: { mode, input } }); draft = await api("/api/prepare", input); $("#status").textContent = draft.opening.isOpen ? "Context verified. Review the interpretation." : "Operating gate closed; silence recorded."; if (!draft.opening.isOpen || draft.interpretation.recommendation !== "develop_direction") { trace("Non-authorising interpretation", draft.interpretation.reason || `Recommendation: ${draft.interpretation.recommendation}`, "system", { interpretation: draft.interpretation }); return; } showInterpretation(); } catch (error) { $("#status").textContent = error.message; } });
...
$("#submit-adapter").onclick = () => withBusy($("#submit-adapter"), "Submitting…", async () => { try { const result = await api(`/api/drafts/${draft.draftId}/submit`); $("#submission-record").classList.remove("hidden"); $("#submission-state").textContent = result.state; $("#submission-request").textContent = result.submission.submittedState.requestId; $("#submission-note").textContent = result.submission.platformState.receipt.platformNote; trace("Adapter submission", "The single publication intent was claimed and finalised; approved / submitted / platform state are recorded separately.", "system", { submission: result.submission }); } catch (error) { $("#verify-result").textContent = error.message; trace("Submission failed", error.message, "error", {}); } });
const architectureStages = [
  ["Trigger → event", "context"], ["Context Builder → verified facts + provenance", "context"],
  ["Philosophy + Strategy → interpretation", "interpretation"], ["Early checkpoint → reject, revise, or continue", "humanGate"],
  ["Creative Direction → bounded candidates", "creativeDirection"], ["Skill Executor → operational artefact", "validation"],
  ["Deterministic validation → facts, format, policy", "validation"], ["Semantic conformance → interpretation fit", "conformance"],
  ["Human approval → exact-state bundle", "approval"], ["Publication adapter → one submission intent", "approval"],
  ["Audit → hash-chained decision trace", "audit"],
];
...
attachSourceReference("#paper-bibliography", "references");
