import { assessOpeningDecision, conformanceCheck, createDirections, interpret, validateArtefact } from "./pipeline.js";
...
$("#prepare").addEventListener("click", () => {
  activeContext = readContext();
  const opening = assessOpeningDecision(activeContext);
  $("#opening-result").textContent = opening.isOpen ? "Opening decision: cart opens. The interpretation can now be reviewed." : "Opening decision: cart stays closed. No Story is prepared.";
  if (!opening.isOpen || activeContext.products.length === 0) {
    $("#review").classList.add("hidden");
    return;
  }
  activeInterpretation = interpret(activeContext);
  ...
  $("#story-image").src = photoUrl;
  renderDirections(createDirections(activeContext, activeInterpretation));
