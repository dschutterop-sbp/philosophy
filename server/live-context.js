...
export async function liveContext(settings, { opensAt, closesAt, products }) {
  const date = localDate(settings.timezone);
  const [conditions, agenda] = await Promise.all([weather(settings), calendar(settings, date)]);
  return { date, day: weekday(date, settings.timezone), opensAt, closesAt, products, ...conditions, ...agenda };
}
