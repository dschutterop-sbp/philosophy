...
async function handleApproveInterpretation(request, response, id) {
  ...
  const next = store.update(draft.id, { state: "interpretation_approved", approvedInterpretation: { by: actor(), at: new Date().toISOString(), reason } }, draft.revision);
...
async function handleApproveArtefact(request, response, id) {
  ...
  const approvalManifest = { draftId: draft.id, decision: "approve_for_publication", interpretationHash: canonicalHash(draft.interpretation), mediaHash: draft.media.hash, textHash: contentHash(direction.caption), directionId: direction.id, accountId: "instagram-story-reference", scheduledTime: null, approver: actor(), approvedAt, expiry, versions: draft.versions };
  const signed = issueApprovalToken(approvalManifest, settings.approvalSigningKey);
  const next = store.update(draft.id, { state: "approved_for_publication", approval: { approvalManifest, ...signed } }, draft.revision);
